...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract recipe JSON from HTML using a model.")
    parser.add_argument("html_file", type=str, help="HTML file to process (default dir: /data/html_pages)")
    parser.add_argument("--model", type=str, default="microsoft/Phi-4-mini-instruct", help="Model to use (default: microsoft/Phi-4-mini-instruct)")
...
"""
    ...
    print(
        f"Loaded model in {time.time() - start:.2f} sec, model size: {model.get_memory_footprint() / 1024 ** 2:.2f} MB device: {model.device}")
